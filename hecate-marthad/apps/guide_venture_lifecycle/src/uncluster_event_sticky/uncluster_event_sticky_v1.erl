%%% @doc uncluster_event_sticky_v1 command
...
-export_type([uncluster_event_sticky_v1/0]).
-opaque uncluster_event_sticky_v1() :: #uncluster_event_sticky_v1{}.
...
-spec validate(uncluster_event_sticky_v1()) -> ok | {error, term()}.
validate(#uncluster_event_sticky_v1{venture_id = V}) when not is_binary(V); V =:= <<>> ->
    {error, {invalid_field, venture_id}};
validate(#uncluster_event_sticky_v1{sticky_id = S}) when not is_binary(S); S =:= <<>> ->
...
-spec from_map(map()) -> {ok, uncluster_event_sticky_v1()} | {error, term()}.
from_map(Map) ->
...
-spec get_venture_id(uncluster_event_sticky_v1()) -> binary().
get_venture_id(#uncluster_event_sticky_v1{venture_id = V}) -> V.

-spec get_sticky_id(uncluster_event_sticky_v1()) -> binary().
get_sticky_id(#uncluster_event_sticky_v1{sticky_id = V}) -> V.
