%%% @doc Projection: phase_started_v1 -> divisions table (phase status update)
%%% Updates the appropriate phase column (dna_status, anp_status, etc.) based
%%% on the "phase" field in the event.
-module(phase_started_v1_to_sqlite_divisions).
...
project(Event) ->
    ...
    query_division_alc_store:execute(Sql, [?PHASE_ACTIVE, DivisionId]).
