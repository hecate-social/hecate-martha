%%% @doc Path utilities for hecate-marthad.
%%%
%%% Provides purpose-specific path functions for the namespaced
%%% directory layout under ~/.hecate/hecate-marthad/:
%%%
...
%%% {hecate_marthad, [{data_dir, Path}]}.
%%% Default: ~/.hecate/hecate-marthad
%%% @end
-module(marthad_paths).
...
base_dir() ->
    case application:get_env(hecate_marthad, data_dir) of
        {ok, Dir} -> expand_path(Dir);
        undefined -> expand_path("~/.hecate/hecate-marthad")
    end.
